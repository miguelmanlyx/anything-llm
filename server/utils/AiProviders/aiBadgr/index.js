...
const cacheFolder = path.resolve(
  process.env.STORAGE_DIR
    ? path.resolve(process.env.STORAGE_DIR, "models", "aibadgr")
    : path.resolve(__dirname, `../../../storage/models/aibadgr`)
);

async function aiBadgrModels(apiKey = null) {
  ...
  try {
    ...
    const openai = new OpenAIApi({
      ...
      apiKey: apiKey || process.env.AIBADGR_API_KEY || null,
    });
...
class AiBadgrLLM {
  constructor(embedder = null, modelPreference = null) {
    ...
    this.model = modelPreference || process.env.AIBADGR_MODEL_PREF;
    this.limits = {
  ...
  async allModelInformation() {
    const models = await aiBadgrModels();
  ...
  async getChatCompletion(messages = null, { temperature = 0.7 }) {
    if (!(await this.isValidChatCompletionModel(this.model)))
      throw new Error(
        `AI Badgr chat: ${this.model} is not valid for chat completion!`
      );
